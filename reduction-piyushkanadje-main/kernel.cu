...
__shared__ float partialSum[2*BLOCK_SIZE];
    ...
    partialSum[t] = in[i + t];
    partialSum[blockDim.x+t] = in[i + blockDim.x+t];
...
__global__ void optimizedReduction(float *out, float *in, unsigned size)
{
 /********************************************************************
    ...
    ********************************************************************/
    ...
    unsigned int i = 2*blockIdx.x * blockDim.x;
    redSum[threadIdx.x] = in[i + threadIdx.x];
    redSum[blockDim.x+threadIdx.x] = in[i + blockDim.x+threadIdx.x];

    for (unsigned int step = blockDim.x; step > 0; step /= 2)
    {
        if (threadIdx.x < step && i+step < size) 
